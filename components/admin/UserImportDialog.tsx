...
import { parseUserExcel, downloadUserTemplate } from "@/lib/utils/excel";
import type { UserImportRow, UserImportResult } from "@/types";
...
export function UserImportDialog({ open, onClose, onSuccess }: UserImportDialogProps) {
  ...
  const [validationResults, setValidationResults] = useState<UserImportResult[]>([]);
  ...
  // 验证数据
  const validateData = async (rows: UserImportRow[]): Promise<UserImportResult[]> => {
    try {
            ...
            <table className="w-full text-sm">
              <thead className="border-b">
                <tr>
                  ...
                  <th className="text-left p-2 font-medium">用户名</th>
                  <th className="text-left p-2 font-medium">真实姓名</th>
                  <th className="text-left p-2 font-medium">角色</th>
                  <th className="text-left p-2 font-medium">电话</th>
                ...
                {data.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-2">{row.real_name}</td>
                      <td className="p-2">{ROLE_NAMES[row.role as keyof typeof ROLE_NAMES] || row.role}</td>
