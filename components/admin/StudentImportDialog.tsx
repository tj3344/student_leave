...
import { parseStudentExcel, downloadStudentTemplate } from "@/lib/utils/excel";
import type { StudentImportRow, StudentImportResult } from "@/types";
...
export function StudentImportDialog({ open, onClose, onSuccess }: StudentImportDialogProps) {
  const [step, setStep] = useState<ImportStep>("upload");
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [file, setFile] = useState<File | null>(null);
  const [data, setData] = useState<StudentImportRow[]>([]);
  const [validationResults, setValidationResults] = useState<StudentImportResult[]>([]);
  ...
  const handleFileSelect = useCallback(
    async (selectedFile: File) => {
      ...
      try {
        // 解析 Excel 文件
        const parsedData = await parseStudentExcel(selectedFile);
  ...
  // 验证数据
  const validateData = async (rows: StudentImportRow[]): Promise<StudentImportResult[]> => {
    try {
            ...
            <table className="w-full text-sm">
              <thead className="border-b">
                <tr>
                  ...
                  <th className="text-left p-2 font-medium">性别</th>
