...
import { parseClassExcel, downloadClassTemplate } from "@/lib/utils/excel";
import type { ClassImportRow, ClassImportResult } from "@/types";
...
export function ClassImportDialog({ open, onClose, onSuccess }: ClassImportDialogProps) {
  const [step, setStep] = useState<ImportStep>("upload");
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [file, setFile] = useState<File | null>(null);
  const [data, setData] = useState<ClassImportRow[]>([]);
  const [validationResults, setValidationResults] = useState<ClassImportResult[]>([]);
  ...
  const handleFileSelect = useCallback(
    async (selectedFile: File) => {
      ...
      try {
        // 解析 Excel 文件
        const parsedData = await parseClassExcel(selectedFile);
  ...
  // 验证数据
  const validateData = async (rows: ClassImportRow[]): Promise<ClassImportResult[]> => {
    try {
            ...
            <table className="w-full text-sm">
              <thead className="border-b">
                <tr>
                  ...
                  <th className="text-left p-2 font-medium">班主任</th>
                ...
                {data.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-2">{row.class_teacher_name || "-"}</td>
                      <td className="p-2">
                        {result?.success ? (
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[700px]">
        <DialogHeader>
          <DialogTitle>导入班级数据</DialogTitle>
          <DialogDescription>
