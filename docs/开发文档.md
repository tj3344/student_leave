...
CREATE TABLE students (
    ...
    address TEXT,
    is_nutrition_meal BOOLEAN DEFAULT FALSE,    -- 是否享受营养餐
...
CREATE TABLE leave_records (
    ...
    review_remark TEXT,
    is_refund BOOLEAN DEFAULT TRUE,         -- 是否退费
...
CREATE TABLE notifications (
    ...
    content TEXT,
    type VARCHAR(50) DEFAULT 'info',       -- system, announcement, reminder, warning
...
#### 4.1.10 数据库连接表 (database_connections)
...
#### 4.1.11 数据库切换历史表 (database_switch_history)
...
CREATE TABLE database_switch_history (
    ...
    switched_by_user_id INTEGER,
    switched_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (from_connection_id) REFERENCES database_connections(id),
    FOREIGN KEY (to_connection_id) REFERENCES database_connections(id),
    FOREIGN KEY (switched_by_user_id) REFERENCES users(id)
);
```
...
| `/api/notifications/class-teachers` | GET | 获取班主任列表 |
| `/api/notifications/send` | POST | 批量发送通知 |
| `/api/notifications/read-all` | POST | 全部标记已读 |
| `/api/admin/sent-notifications/batches` | GET | 获取已发送通知批次 |
| `/api/admin/notifications/cleanup` | POST | 清理已读通知 |
...
| `/api/database/connections` | POST | 创建数据库连接 |
| `/api/database/connections/[id]` | GET | 数据库连接详情 |
| `/api/database/connections/[id]` | PUT | 更新数据库连接 |
| `/api/database/connections/[id]` | DELETE | 删除数据库连接 |
| `/api/database/connections/[id]/test` | POST | 测试数据库连接 |
| `/api/database/connections/[id]/switch` | POST | 切换数据库 |
| `/api/database/status` | GET | 获取当前数据库状态 |
| `/api/database/history` | GET | 获取数据库切换历史 |
...
| `/admin/notifications` | 通知管理 | 接收的通知列表 |
| `/admin/sent-notifications` | 已发送通知 | 发送的通知记录 |
| `/admin/database` | 数据库管理 | 数据库连接管理 |
...
lib/api/notifications.ts   # 通知服务
lib/api/database.ts        # 数据库管理服务
...
```env
# 数据库
DATABASE_URL=postgresql://user:password@localhost:5432/student_leave
